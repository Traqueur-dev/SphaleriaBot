...
public class InviteRemoveCommand extends ICommand {

    @Command(name = {"invite.remove","invites.remove", "invites.rm", "invite.rm", "invites.delete", "invite.delete"},
            permittedRoles = {"537682405893341185", "537682450407620637"},
            permittedUsers = "285375027480756224", description = "Permet d'enlever des invitations bonus à un joueur")
    public void onCommand(CommandArgs args) {
        ProfileManager manager = ProfileManager.getInstance();
        TextChannel channel = args.getChannel();
        if (args.size() != 2) {
            EmbedBuilder builder = args.getInfo();
            builder.addField("Usage:", args.getPrefix() + "invite remove <user> <nombre>", true);
        ...
        boolean isAdd = manager.removeInvite(target, num);
        EmbedBuilder builder;
        if (isAdd) {
            builder = args.getSuccess();
            builder.addField("Succés:", args.getSender() + " vient de retirer " + num
